...
// NOTE: 修改
using match_t = std::pair<std::vector<std::vector<Sales_data>>::size_type,
...
std::vector<match_t> findBook(const std::vector<std::vector<Sales_data>> &files, const std::string &book)
{
    ...
    for (auto iter = files.cbegin(); iter != files.cend(); ++iter)
    {
        ...
        {
            result.push_back({iter - files.cbegin(), {found.first, found.second}}); // NOTE: 修改
        }
    }
...
void reportResult(std::istream &is, std::ostream &os, const std::vector<std::vector<Sales_data>> &files)
{
    ...
    while (is >> book)
    {
        ...
        {
            // NOTE: 修改
            os << "store" << store.first + 1
               << " sales: " << std::accumulate(store.second.first, store.second.second, Sales_data(book)) << std::endl;
        }
    }
}
